...
let runFuncFormArray = (funcList: Array<Function>, param?: any) => {
    try {
        ...
        //console.log(JSON.stringify(error),"haha");
    }
 ...
 * SDK接入, 微信信息注册
 */
export default class Native_IOS {
    /** 用户信息 */
    ...
    public static init(onShowCallBack, onHideCallback){
        ...
        // //console.log("[hd_sdk_nativeIOS]----->设备信息 ", systemInfo);
        let systemInfo = JsBridgeIOS.getDevicesInfo();
        //console.log('[hd_sdk_nativeIOS]------->设备信息', systemInfo);
    ...
    public static rigisterOnStartEvent(){
        cc.game.on(cc.game.EVENT_GAME_INITED, (res) => {
            //console.log('[hd_sdk_nativeIOS]----->EVENT_GAME_INITED', res)
            /** 初始化JSB */
            JsBridgeIOS.init();
    ...
    public static onShowCallBack(res){
        //console.log('[hd_sdk_nativeIOS]----->onshow', res)

        /** onShowRes */
        // //console.log('[hd_sdk_nativeIOS]------>onShowCallBack', res);
        // this.onShowRes = res;
        ...
        //         //console.log('[hd_sdk_nativeIOS]----->开始上报交叉推广,从' + fromAppId + "跳入");
        //         HD_MODULE.getNet().postDSPIn({from_app_id: fromAppId}, () => {
        //             //console.log('[hd_sdk_nativeIOS]----->上报交叉推广成功,从' + fromAppId + "跳入");
        //         });
        //     }
        // }
        ...
        //         //console.log('[hd_sdk_nativeIOS]----->获取分享开关', res.data.switch);
    ...
    public static loginCallBack(res){
        ...
        //console.log('[hd_sdk_nativeIOS]----------------->登录成功');
        this.isLogin = true;
    ...
    public static share(data: wx.types.ShareOption){
        ...
        //console.log('[hd_sdk_nativeIOS]----->分享', imageUrl, shareMsg);
   ...
   public static showAlert(title: string, content: string, okText: string = "确定", cancelText: string = ""){
    ...
    //     if(this.userInfo){
    //         if(success){
    //             //console.log('[hd_sdk_nativeIOS]----->获取用户信息', this.userInfo);
	...
	private static _silenceLogin(success: Function = () => {}, fail: Function = () => {}){
        Native_IOS.loginFailCallBackList.push(fail);
        if (Native_IOS.loginCode != 0) {
            Native_IOS.loginCallBackList.push(success);
            // //console.log('[登录队列1]', Native_IOS.loginCallBackList.length, Native_IOS.loginFailCallBackList.length)
            if (Native_IOS.loginCallBackList.length > 1) {
                return;
            }
        }
        ...
        // /** 重连 */
        // let reconnect = () => {
        //     //console.log('[hd_sdk_nativeIOS]------>静默登录失败');
        //     let reconnectTimerID = null;
        //     let reconnectCount = 5;
        //     reconnectTimerID = setInterval(() => {
        //         //console.log('[hd_sdk_nativeIOS]------>', reconnectCount, '秒后尝试重连');
        //         reconnectCount--;
        //         if(reconnectCount == 0) this._silenceLogin(() => {clearInterval(reconnectTimerID);}, () => {reconnect()});
        //     }, 1000);
        // }
        
        let loginSuc = (res: typeof ocres) => {
            //console.log('[hd_sdk_nativeIOS]------>用户信息', res.email, res.memberId, res.mobile, res.nickName, res.userName, res.userToken);
            {
                // HDDefaultUserInfo.avatar_url = this.userInfo.avatarUrl;
                // HDDefaultUserInfo.city = this.userInfo.city;
                HDDefaultUserInfo.code = "sh";
                // HDDefaultUserInfo.country = this.userInfo.country;
                // HDDefaultUserInfo.gender = this.userInfo.gender;
                HDDefaultUserInfo.nick_name = res.nickName;
                HDDefaultUserInfo.open_id = res.memberId;
                // HDDefaultUserInfo.province = this.userInfo.province;
                HDDefaultUserInfo.code = res.userToken;
            }
            this.userInfo = HDDefaultUserInfo;
            if(!HD_MODULE.getPlatform().getToken()){
                //console.log("[hd_sdk_nativeIOS]------>本地token为空或者已过期,重新登录小游戏服务器:", appConfig.platform_id, appConfig.app_id, appConfig.env);
                let params = {
                    platform_id: appConfig.platform_id,
                    app_id: appConfig.app_id,
                    user_info: HDDefaultUserInfo
                }
                HD_MODULE.getNet().postMiniGameLogin(params, (recv) => {
                    let token = recv.data.token;
                    //console.log('[hd_sdk_nativeIOS]------>登录小游戏服务器', token);
                    HD_MODULE.getPlatform().setToken(token);
                    localStorage.setItem("token" + appConfig.platform_id + appConfig.env, token);
                    Native_IOS.loginCode = 1;
                    Native_IOS.loginCallBack(res);
                }, (err) => {
                    console.error('[hd_sdk_nativeIOS]------>登录小游戏服务器失败', JSON.stringify(err));
                    Native_IOS.loginCode = 0;
                    Native_IOS.loginFailCallBack(err);
                    if(fail)
                        fail(err);
                });
            }
        ...
        JsBridgeIOS.agentSDKLogin(loginSuc);
	}
	...
	private static _login(success: Function, fail?: Function, noAuth?: boolean){
		wx.login({
            success: (res) => {
                if (res.code) {
					let codeRes = res;
                    //console.log("[hd_sdk_nativeIOS]------>大授权登录:", res);
                    ...
                    let btnLogin = () => {
                        //console.log("[hd_sdk_nativeIOS]------>创建按钮授权");
                    ...
                    wx.getSetting({
                        success: (res) => {
                            //console.log('[hd_sdk_nativeIOS]----->获取授权信息', res);
                    ...
                    console.error("[hd_sdk_nativeIOS]------>微信登录失败:", res);
                }
            },
        });
    }
        ...
        if(!this.shareTemplate){
            // if(!this.isGetShareTemplate){
                ...
                HD_MODULE.getNet().getShareTemplate({version: appConfig.app_version}).then((res) => {
                    //console.log('[hd_sdk_nativeIOS]----->获取分享模板', res);
    ...
    public static openShareByShareTemplateRand(type: string, data?: wx.types.ShareOption){
        this.getOneShareInfoByType(type, (shareInfo) => {
            if(shareInfo){
                ...
                //console.log('[hd_sdk_nativeIOS]----------------->随机素材分享', shareData)
            }
        });
    }
    ...
    //         //         //console.log('[hd_sdk_nativeIOS]----->视频正常播放结束');
    //         //         EventManager.emit('video-success');
    //         //         if(success) success();
    //         //     } else {
    //         //         //console.log('[hd_sdk_nativeIOS]----->视频中途播放结束或视频错误');
    //         //         if(fail) fail(res);
    //         //     }
    ...
    static inAppPurchaseWithOrderInfo(): Promise<any>{
        return new Promise((resolve) => {
            JsBridgeIOS.inAppPurchaseWithOrderInfo().then(resolve);
        });
    }
}
